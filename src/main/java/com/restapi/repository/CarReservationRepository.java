...
public interface CarReservationRepository extends JpaRepository<CarReservation, Integer> {
    ...
    List<CarReservation> findAllUpcomingReservationsOfUser(@Param("user_id") Integer userId, @Param("current_date") LocalDateTime currentDate);

    @Query("SELECT c FROM CarReservation c WHERE c.appUser.id = :user_id AND c.end_date < :current_date")
    List<CarReservation> findAllPastReservationsOfUser(@Param("user_id") Integer userId, @Param("current_date") LocalDateTime currentDate);
    ...
    List<CarReservation> findAllCurrentReservationsOfUser(@Param("user_id") Integer userId, @Param("current_date") LocalDateTime currentDate);
